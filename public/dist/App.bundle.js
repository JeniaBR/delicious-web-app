...
function autocomplete(input, latInput, lngInput) {
  ...
  //if someone hits ender on the address field, don't submit the form
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    // hit our API via axios and handle the results with searchResultsHTML
    _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
      ...
      searchResult.innerHTML = _dompurify2.default.sanitize('<div class="search__result">No results for ' + _this.value + ' found!</div>');
    }).catch(function (err) {
      console.error(err);
    });
  });
  ...
  searchInput.on('keyup', function (e) {
    // only check for up, down, enter keys
